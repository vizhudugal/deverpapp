...
exports.fetch_item_dao_fn =function(req,res,result){
  ...
  .where('item_name',req.name).then(function (status) {
      if (status.length>0) {
        result(status);
      }
      else{
        bookshelf.knex.select('*').from('md_sales_finishedgoods_item')
        .leftJoin('md_item_item_type','md_sales_finishedgoods_item.item_type_id','md_item_item_type_detail.item_type_id')
