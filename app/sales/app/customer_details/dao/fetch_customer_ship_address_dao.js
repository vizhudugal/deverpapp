...
exports.fetch_customer_ship_address_dao_fn = function(customer,res,result){
    console.log("ship add dao"+JSON.stringify(customer));
    bookshelf.knex('md_sales_customer as t1').select('t3.*','t4.*','t5.state_name')
    .innerJoin('od_customer_ship_mapping as t2', 't2.customer_id', 't1.customer_id')
    .innerJoin('od_sales_customer_shipping_address as t3', 't3.shipping_id', 't2.shipping_id')
